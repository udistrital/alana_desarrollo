...
if ($_REQUEST ['funcion'] == 'consultarInfoSociedadTemporal') {
    ...
    $resultadoItems = $esteRecursoDBAgora->ejecutarAcceso($cadenaSql, "busqueda");
    $sqlNombreRepresentante = $this->sql->getCadenaSql("nombre_participante_natural", $resultadoItems[0]['documento_representante']);
    $nombreRepresentante = $esteRecursoDBAgora->ejecutarAcceso($sqlNombreRepresentante, "busqueda");
    $sqlNombreRepresentanteSuplente = $this->sql->getCadenaSql("nombre_participante_natural", $resultadoItems[0]['documento_suplente']);
    $nombreRepresentanteSuplente = $esteRecursoDBAgora->ejecutarAcceso($sqlNombreRepresentanteSuplente, "busqueda");
    $resultadoItems[0][4] = $nombreRepresentante[0][0];
    $resultadoItems[0][5] = $nombreRepresentanteSuplente[0][0];
    ...
    for ($i = 0; $i < count($participantes); $i++) {
        $sqlNombreParticipante = $this->sql->getCadenaSql("nombre_participante", $participantes[$i][2]);
        $nombreParticipante = $esteRecursoDBAgora->ejecutarAcceso($sqlNombreParticipante, "busqueda");
        $participantes[$i][2] = $nombreParticipante[0][0];
    }
...
//-----------------------Consultar por Idnetificacion de Sociedad
if ($_REQUEST ['funcion'] == 'consultaIdentificacionSociedad') {
    $cadenaSql = $this->sql->getCadenaSql('buscar_sociedad_identificacion', $_GET ['query']);
    $resultadoItems = $esteRecursoDBAgora->ejecutarAcceso($cadenaSql, "busqueda");
...
//---------Consultar Ciudad por Departamento------------------
...
//----Consulta Representante Titular y Representante Suplente 

if ($_REQUEST ['funcion'] == 'ObtenerRepresentantes') {

    $cadenaSql = $this->sql->getCadenaSql('buscar_representante', $_GET ['query']);
    $resultadoItems = $esteRecursoDBAgora->ejecutarAcceso($cadenaSql, "busqueda");
...
// -----------------------Obtener Participantes Sociedad Temporal
